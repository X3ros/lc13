...
/obj/item/ego_weapon/black_silence_gloves/ranga/proc/dash_attack(mob/living/user, atom/target)
	...
	L.deal_damage(80, BLACK_DAMAGE, user, attack_type = (ATTACK_TYPE_MELEE | ATTACK_TYPE_SPECIAL))
	exchange_cooldown -= 20
	switch(dash_count)
		if(0)
...
/obj/item/ego_weapon/black_silence_gloves/mook/Special(mob/living/user, atom/target)
	...
	if(stacks > 0)
		...
		for (var/i = 0; i < 3; i++)
			...
			for(var/mob/living/L in user.HurtInTurf(T, list(), 50, BLACK_DAMAGE, check_faction = TRUE, hurt_mechs = TRUE, attack_type = (ATTACK_TYPE_MELEE | ATTACK_TYPE_SPECIAL)))
				exchange_cooldown -= 10
			sleep(0.25 SECONDS)
...
/obj/item/ego_weapon/black_silence_gloves/logic/Special(mob/living/user, atom/target)
	...
	for(var/turf/T in area_of_effect)
		new /obj/effect/temp_visual/smash_effect(T)
		var/list/new_hits = user.HurtInTurf(T, been_hit, 100, BLACK_DAMAGE, check_faction = TRUE, hurt_mechs = TRUE, attack_type = (ATTACK_TYPE_RANGED | ATTACK_TYPE_SPECIAL)) - been_hit
		been_hit += new_hits
...
/obj/item/ego_weapon/black_silence_gloves/crystal/proc/dash_attack(mob/living/user, atom/target)
	...
	if(dash_count < 1)
		L.deal_damage(60, BLACK_DAMAGE, user, attack_type = (ATTACK_TYPE_MELEE | ATTACK_TYPE_SPECIAL))
		addtimer(CALLBACK(src, PROC_REF(dash_attack), user, target), 5)
		new /obj/effect/temp_visual/smash_effect(F)
		dash_count += 1
	else
		L.deal_damage(100, BLACK_DAMAGE, user, attack_type = (ATTACK_TYPE_MELEE | ATTACK_TYPE_SPECIAL))
		new /obj/effect/temp_visual/smash_effect(F)
		exchange_cooldown -= 30
		dash_count = 0
...
/obj/item/ego_weapon/black_silence_gloves/wheels/Special(mob/living/user, atom/target)
	...
	if(do_after(user, 10, target))
		...
		for(var/turf/T in area_of_effect)
			new /obj/effect/temp_visual/smash_effect(T)
			var/list/new_hits = user.HurtInTurf(T, been_hit, 300, BLACK_DAMAGE, check_faction = TRUE, hurt_mechs = TRUE, attack_type = (ATTACK_TYPE_MELEE | ATTACK_TYPE_SPECIAL)) - been_hit
			been_hit += new_hits
			for(var/mob/living/L in new_hits)
...
/obj/item/ego_weapon/black_silence_gloves/proc/furioso(mob/living/user, mob/living/target)
	...
	if(!target.anchored)
		target.Move(target_turf)
	L.deal_damage(1500, BLACK_DAMAGE, user, attack_type = (ATTACK_TYPE_MELEE | ATTACK_TYPE_RANGED | ATTACK_TYPE_SPECIAL)) //this went on for 5 sec, so 300 DPS as the final attack
	sleep(10)
