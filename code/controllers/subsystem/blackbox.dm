...
/datum/controller/subsystem/blackbox/proc/LogBroadcast(freq)
	...
	switch(freq)
		...
		if(FREQ_CONTROL)
			record_feedback("tally", "radio_usage", 1, "control")
		if(FREQ_INFORMATION)
			record_feedback("tally", "radio_usage", 1, "information")
		if(FREQ_SAFETY)
			record_feedback("tally", "radio_usage", 1, "safety")
		if(FREQ_TRAINING)
			record_feedback("tally", "radio_usage", 1, "training")
		if(FREQ_COMMAND)
			record_feedback("tally", "radio_usage", 1, "command")
		if(FREQ_WELFARE)
			record_feedback("tally", "radio_usage", 1, "welfare")
		if(FREQ_DISCIPLINE)
			record_feedback("tally", "radio_usage", 1, "discipline")
		if(FREQ_RECORDS)
			record_feedback("tally, radio_usage", 1, "records")
		if(FREQ_EXTRACTION)
			record_feedback("tally", "radio_usage", 1, "extraction")
		if(FREQ_ARCHITECTURE)
			record_feedback("tally", "radio_usage", 1, "architecture")
		if(FREQ_SYNDICATE)
		...
		if(FREQ_HEAD)
			record_feedback("tally", "radio_usage", 1, "head")
		if(FREQ_CTF_RED)
