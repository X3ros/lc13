GLOBAL_LIST_INIT(arcade_prize_pool, list(
		...
		/obj/item/clothing/under/mercenary/tacticool = 2,
...
/obj/machinery/computer/arcade/proc/prizevend(mob/user, prizes = 1)
	...
	for(var/i = 0, i < prizes, i++)
		if(prob(0.0001)) //1 in a million
			...
			user.client.give_award(/datum/award/achievement/ss13/pulse, user)
			return
...
/obj/machinery/computer/arcade/battle/proc/gameover_check(mob/user)
	...
	if(enemy_hp <= 0)
		if(!gameover)
			...
			if(obj_flags & EMAGGED)
				new /obj/item/grenade/syndieminibomb(loc)
...
/obj/machinery/computer/arcade/orion_trail/proc/report_player(mob/gamer)
	...
	if(gamers[gamer] > 2 && prob(20 * gamers[gamer]))
		...
		Radio.talk_into(src, "SECURITY ALERT: Crewmember [gamer] recorded displaying antisocial tendencies in [get_area(src)]. Please watch for violent behavior.", FREQ_DISCIPLINE)

		Radio.set_frequency(FREQ_WELFARE)
		Radio.talk_into(src, "PSYCH ALERT: Crewmember [gamer] recorded displaying antisocial tendencies in [get_area(src)]. Please schedule psych evaluation.", FREQ_WELFARE)
