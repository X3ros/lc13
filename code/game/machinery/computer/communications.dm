...
/obj/machinery/computer/communications/proc/authenticated_as_non_silicon_captain(mob/user)
	if (issilicon(user))
		return FALSE
	return ACCESS_MANAGER in authorize_access
...
/obj/machinery/computer/communications/ui_act(action, list/params)
	...
	switch (action)
		...
		if ("purchaseShuttle")
			...
			minor_announce("[usr.real_name] has purchased [shuttle.name] for [shuttle.credit_cost] ahn.[shuttle.extra_desc ? " [shuttle.extra_desc]" : ""]" , "Shuttle Purchase")
