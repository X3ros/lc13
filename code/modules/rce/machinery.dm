...
/obj/machinery/factory_machine/cross/process(delta_time)
	...
	var/move_amt = 0
	for(var/O in directional_storage)
		move_amt++
		if(!O || isnull(O))
			directional_storage -= O
			continue
		if(!directional_storage[O] || get_turf(O) != get_turf(src) || O == src)
			directional_storage -= O
			continue
	...
	for(var/mob/living/carbon/patient in oview(get_turf(src), 3))
		...
		if(patient)
			...
			if(patient.stat >= SOFT_CRIT)
				continue
			//If patient is damaged then restore 3 health as a default regenerator would.
