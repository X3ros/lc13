	...
	if(isfloorturf(exposed_turf))
		...
		if(isfloorturf(target_floor))
			for(var/turf/nearby_turf in range(1, target_floor))
				if(!locate(/obj/effect/turf_fire) in nearby_turf)
					new /obj/effect/turf_fire(nearby_turf)
...
/datum/reagent/clf3/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume)
	...
	if(!locate(/obj/effect/turf_fire) in exposed_mob.loc)
		new /obj/effect/turf_fire(exposed_mob.loc)
...
/datum/reagent/firefighting_foam/expose_turf(turf/open/exposed_turf, reac_volume)
	...
	var/obj/effect/turf_fire/flames = (locate(/obj/effect/turf_fire) in exposed_turf)
	if(flames)
		qdel(flames)
